 ...
 */
  mount(mountInfoList) {
    ...
    if (!mountInfoList) {
      for (let name in this.contracts) {
        ...
        let endpoint = `[${contract.method}]:[${contract.url}]`;
        let mountedName = mountedInfo[endpoint];
        assert(!mountedName, `fatal error: ${endpoint} has been used by ${mountedName}`);

        router[contract.method](contract.url, this._createRouteProcessor(contract));
        this.logger.info(`loaded contract ${contract.name} at ${endpoint}`);

        mountedInfo[endpoint] = contract.name;
      }
    } else {
  ...
  _createRouteProcessor(contract) {
    ...
    return function* (next) {
      ...
      result = contract.processResult(result);
