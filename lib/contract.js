...
class Contract {
  constructor(definition) {
    ...
    assert(_isGeneratorFunction(definition.processor), 'contract.processor is not generator function');
  ...
  getRuntimeParamsValue(query, params, body, headers) {
    ...
    _(this.params).forEach((paramDef, paramName) => {
      ...
      if (paramDef.from === 'query') {
        ...
        paramValue = koaThis.request.body[paramName];
        tryConvert = false;
      } else if (paramDef.from === '@body') {
        paramValue = koaThis.request.body;
        tryConvert = false;
      } else if (paramDef.from === 'header') {
        paramValue = koaThis.headers[paramName];
      } else {
...
function _checkValue(type, value) {
  if (_isValidPrimType(type)) {
    ...
    if (_isValidPrimValue(type.TYPE, value)) {
      return;
    } else if (value === undefined && type.require === false) { //值不存在，但也不是必须的
      return;
    } else {
...
function _isGeneratorFunction(obj) {
  if (!obj) return false;
...
exports = module.exports = function(definition) {
